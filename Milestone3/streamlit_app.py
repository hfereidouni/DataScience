...
def fetch_and_display_game_data(game_id):
    ...
    if game_id:
        ...
        data = game_cli.ping_game(game_id)
        ...
        data['xG'] = pd.DataFrame(predicted_xG.items())[1]
        ...
        cols = st.columns(2)
        #cols[0].metric(label=f"{home_team} xG (Actual Score)", value=f"{home_xG} ({home_score})", delta=int(home_score - home_xG))
        #cols[1].metric(label=f"{away_team} xG (Actual Score)", value=f"{away_xG} ({away_score})", delta=int(away_score - away_xG))

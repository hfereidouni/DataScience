...
def calc_percentile_goal_rate_data_display_random(class_probabilities, Y_valid, BASELINE_FOLDER_PATH, titlename, modelname, name_placeholder, experiment):
    ...
    probas_and_label['rank'] = probas_and_label['goal_proba'].rank(method="first", pct=True)*100
...
def display_calibration_from_pred(clf,X_valid, Y_valid, BASELINE_FOLDER_PATH, titlename, modelname, name_placeholder, experiment):

    # disp = CalibrationDisplay.from_predictions(Y_valid.to_numpy(), class_probabilities[:,1])
    
    prob_true, prob_pred = calibration_curve(Y_valid, clf.predict(X_valid))
    y_prob = clf.predict_proba(X_valid)
    disp = CalibrationDisplay(prob_true, prob_pred, y_prob)
    disp = disp.plot(label="Random Classifer")
...
def random_uniform_classifier(X_train, X_valid, Y_train, Y_valid, features_list, BASELINE_FOLDER_PATH, BASELINE_MODEL_DIR, experiment, name_placeholder="train_cols", titlename="train_cols"):
    ...
    # Goal rate v/s Shot probability percentile
    goal_rate_plot, cum_goal_rate_plot = calc_percentile_goal_rate_data_display_random(class_probabilities, Y_valid, BASELINE_FOLDER_PATH, titlename, modelname, name_placeholder, experiment)

    #Calibration Display   
    cali_plot = display_calibration_from_pred(dummy_clf, X_valid, Y_valid, BASELINE_FOLDER_PATH, titlename, modelname, name_placeholder, experiment)
...
def experiment_setup(features_for_model, model_code, name_placeholder, titlename, BASELINE_FOLDER_PATH, BASELINE_MODEL_DIR):
    ...
    # Train Classifier
    roc_plot,goal_rate_plot,cum_goal_rate_plot,cali_plot, auc = models[model_code](
                                X_train,
                                X_valid,
                                Y_train,
                                Y_valid,
